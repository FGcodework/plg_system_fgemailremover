...
class PlgSystemFgemailremover extends JPlugin
{
    ...
    public function onAfterRender()
    {
        ...
        // "@" (mailto: links, plain-text addresses) nor anything that
        // could be a classic Joomla email.cloak span (which encodes the
        // address as JS string concatenation with numeric HTML entities
        // - never a literal "@" - so it needs its own check here). Just
        // "cloak" rather than a specific "id=\"cloak" substring, since
        // the real detection in stripClassicCloakElements() is lenient
        // about attribute order and quote style - this early check only
        // needs to avoid false negatives, not be precise; a rare false
        // positive here just means doing a bit of unnecessary work, not
        // an incorrect result.
        if (strpos($buffer, '@') === false && stripos($buffer, 'cloak') === false) {
            return;
        }
